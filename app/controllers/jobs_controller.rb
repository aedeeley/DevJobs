class JobsController < ApplicationController
  ...
  def index
    ...
    if(params.has_key?(:job_type))
      @jobs = Job.paginate(:page => params[:page], :per_page => 5).where(job_type: params[:job_type]).order("created_at desc")
    else
  ...
  def create
    ...
    respond_to do |format|
      if @job.save
        format.html { redirect_to @job, notice: 'Your purchase was successful and the job was posted!' }
